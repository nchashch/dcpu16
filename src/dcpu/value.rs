...
pub enum Value {
    ...
    IndexReg(Register, u16), // [register + next word]
    ...
    PC,
    EX,
    DerefNextWord(u16), // [next word]
    NextWord(u16), // next word (literal)
...
impl Value {
    pub fn new(val: u16) -> Option<Value> {
        if val <= 0x07 {
            ...
            if let Some(reg) = Register::new(val - 0x10) {
                return Some(Value::IndexReg(reg, 0));
            }
        ...
        match val {
            ...
            0x1c => Some(Value::PC),
            0x1d => Some(Value::EX),
            0x1e => Some(Value::DerefNextWord(0)),
            0x1f => Some(Value::NextWord(0)),
            _ => None
        }
    }
    ...
    pub fn code(&self) -> u16 {
        match self {
            ...
            Value::IndexReg(reg, _) => 0x10 + reg.code(),
            Value::STACK => 0x18,
            Value::PEEK => 0x19,
            Value::PICK => 0x1a,
            Value::SP => 0x1b,
            Value::PC => 0x1c,
            Value::EX => 0x1d,
            Value::DerefNextWord(_) => 0x1e,
            Value::NextWord(_) => 0x1f,
